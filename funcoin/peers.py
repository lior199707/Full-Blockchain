...
class P2PProtocol:
    ...
    async def handle_ping(self, message, writer):
        ...
        # Let's send them blocks if they have less than us(they're not up to date)
        if block_height < self.blockchain.last_block["height"]:
            # Send them each block in the succession, from their height up.
    ...
    async def handle_transaction(self, message, writer):
       ...
       :param message: message object
       :param writer: asyncio.StreamWriter, the user
        """
        ...
        if validate_transaction(tx) is True:
            ...
            if tx not in self.blockchain.pending_transactions:
                ...
                for peer in self.connection_pool.get_alive_peers(20):
                    await self.send_message(
                        peer,
                        create_transaction_message(self.server.external_ip, self.server.external_port, tx),
                    )
            else:
